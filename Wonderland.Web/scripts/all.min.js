...
wonderlandApp.config([ "$provide", function($provide) {
...
} ]), wonderlandApp.controller("PartyCtrl", [ "safeApply", "$filter", "$ocModal", "$rootScope", "$sce", "$scope", "$timeout", function(safeApply, $filter, $ocModal, $rootScope, $sce, $scope) {
    ...
    $scope.partyDetailsDataForForm = null, $scope.partyDetailsDataInit = function(partyDetailsData) {
        ...
        console.log("deets", response), $scope.partyDetailsDataForForm.processing = !1, 
        response && response.Success === !0 ? (dataObject.error = null, $scope.partyDetailsDataInit(dataObject), 
        $ocModal.close("partyDetailsModal")) : response.Errors && ($scope.partyDetailsDataForForm.error = response.Errors[0].ErrorMessages[0]);
    ...
    $scope.partyCustomImage = {
        ...
        response && response.Success === !0 && response.Message && safeApply($scope, function() {
    ...
    return {
        ...
        template: '<div><div class="btn-facebook-login radius-v2-small" ng-click="login()" ng-class="{\'disabled\': loading}" ng-bind="label"></div><div class="loader pos-static" ng-show="loading"></div><div class="successful" ng-show="formattedDetails">The user has logged in via Facebook! Hendy at this stage, we need to log the users in using their facebook app id.</div><div class="boxx internal highlight radius-v3-small" ng-show="formattedDetails"><table><tr ng-repeat="deet in formattedDetails"><td ng-bind="deet.key"></td><td ng-bind="deet.value"></td></tr></table></div></div>',
        link: function(scope) {
            scope.loading = !1, scope.formattedDetails = null;
            var fbStatus = null, fbLogin = null, fbDetails = null, getUserDetails = function() {
                fbDetails = facebook.getUserDetails(), fbDetails.then(function() {
                    scope.loading = !1, formatUserDetails();
                });
                ...
                }), deets.length && (scope.formattedDetails = deets);
            };
            scope.login = function() {
                scope.loading || (scope.loading = !0, fbStatus.then(function(status) {
                    "connected" !== status ? (fbLogin = facebook.doLogin(), fbLogin.then(function(newStatus) {
                        "connected" !== newStatus ? scope.loading = !1 : getUserDetails();
                    })) : getUserDetails();
                }));
    ...
    return {
        ...
        link: function(scope, element, attrs) {
            ...
            setProgressState("ready"), element.on("submit", function() {
                function postTo() {
                    if (postDeferred = $q.defer(), !$postForm) {
                        ...
                        }).insertAfter(element), element.appendTo($postForm);
                    }
                    return $postForm.attr("action", attrs.action), $postForm.submit(), postDeferred.promise;
                }
                var postDeferred, $targetIframe = null, $postForm = null;
                return postTo(), !1;
                ...
                return safeApply(scope, function() {
                    if (!serialized || !mHttp.isInProgress(formSubmitRequest)) {
                        ...
                        var originalData = dataVar(scope) || {}, sendData = angular.copy(originalData), ufprtElement = element.find("[name='ufprt']");
        ...
        link: function(scope, element, attrs) {
            ...
            var loadImage = function(newImgSrc) {
                ...
                $(img).on("load", function() {
                    element.css("background-image", "url(" + newImgSrc + ")"), safeApply(scope, function() {
    ...
    return {
        ...
        link: function(scope, element) {
            scope.toggle = !1, $document.bind("click", function(event) {
                ...
                isClickedElementChildOfPopup || safeApply(scope, function() {
                    scope.toggle = !1;
                });
            });
        }
...
}), wonderlandApp.factory("mHttp", [ "$compile", "$document", "$q", "$rootScope", "$window", "mHttpBackend", "safeApply", "uniqueId", function($compile, $document, $q, $rootScope, $window, mHttpBackend, safeApply, uniqueId) {
    var cancelFns = {}, mHttp = function(opts) {
        function onResponse(args, success) {
            ...
            optsWithDefaults.invokeApply && safeApply($rootScope);
        }
        function cleanup() {
