...
wonderlandApp.config([ "$provide", function($provide) {
    ...
    $scope.partyDetailsDataForForm = null, $scope.partyDetailsDataFeedback = {}, $scope.partyDetailsDataInit = function(partyDetailsData) {
        ...
        }), console.log("partyDetailsData", partyDetailsData), partyDetailsData && partyDetailsData.PartyDateTime && (partyDetailsData.PartyDateTime = moment(partyDetailsData.PartyDateTime).format("YYYY/MM/DD HH:mm:SS"));
        var address = [];
    ...
    $scope.suggestedDonationDataInit = function(suggestedDonationData) {
        ...
        $scope.suggestedDonationDataInit(dataObject), $ocModal.close("suggestedDonationModal");
    ...
    var initUrl = null;
    $scope.partyImageData = null, $scope.partyImageDefaultData = null, $scope.partyImageDataForForm = null, 
    $scope.partyImageDataFeedback = {}, $scope.partyCustomImage = {
        url: null
    }, $scope.partyImageDefaultDataInit = function(defaultImages) {
        $scope.partyImageDefaultData = defaultImages, $scope.partyImageDefaultData.indexOf(initUrl) >= 0 || ($scope.partyCustomImage.url = initUrl);
        ...
        response && response.Success === !0 && response.Message && $timeout(function() {
            $scope.partyCustomImage.url = response.Message, $scope.partyImageDataForForm.PartyImage = $scope.partyCustomImage.url;
        });
    }), $scope.hardCodedCurrentPartyImageUrlInit = function(url) {
        initUrl = url, $scope.partyImageData = {
            PartyImage: url
    ...
    $scope.wall = {
        ...
        $timeout(function() {
            $scope.wall.feedback.imageProcessing = !1, response && response.Success === !0 && response.Message ? ($scope.wall.feedback.imageShowSuccess = !0, 
            $scope.wall.previewImageUrl = response.Message, $scope.wall.formModel.PartyWallImage = $scope.wall.previewImageUrl.replace("/Uploads/PartyWall/", "")) : $scope.wall.feedback.imageShowError = !0, 
            $timeout(function() {
    ...
    return {
        ...
        link: function(scope, element, attrs) {
            ...
            var progressState, progressVar = attrs.mProgressVar && $parse(attrs.mProgressVar), dataVar = $parse(attrs.mPostOnSubmit), formSubmitRequest = null, savedConfirmTimer = null, serialized = attrs.hasOwnProperty("mSerialized"), onSuccessEvent = attrs.mOnSuccessEvent, onErrorEvent = attrs.mOnErrorEvent;
            setProgressState("ready"), element.on("submit", function() {
                if (!serialized || !mHttp.isInProgress(formSubmitRequest)) {
                    ...
                    if (window.File && window.FileReader) $timeout(function() {
                        ...
                        formSubmitRequest = mHttp.post(attrs.action + "?t=" + new Date().getTime(), {
                            formData: sendData,
                            dataType: "json"
                        }), formSubmitRequest.then(function() {
                        ...
                        element.appendTo($postForm), console.log("element", $(element).serialize()), console.log("postForm", $postForm.serialize()), 
                        $postForm.attr("action", attrs.action), $postForm.submit();
                    }
                    return !1;
                }
            });
