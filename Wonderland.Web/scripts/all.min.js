...
!function(a) {
    ...
    "function" == typeof define && define.amd ? define([ "outlayer/outlayer", "get-size/get-size" ], masonryDefinition) : window.Masonry = masonryDefinition(window.Outlayer, window.getSize);
}(window), function() {
    function EventEmitter() {}
    function indexOfListener(listeners, listener) {
        for (var i = listeners.length; i--; ) if (listeners[i].listener === listener) return i;
...
wonderlandApp.config([ "$provide", function($provide) {
    ...
    $scope.fundraisingTargetDataInit = function(fundraisingTargetData) {
        $timeout(function() {
            ...
            for (var x in $scope.fundraisingTargetDataForForm) console.log(x + " : ", $scope.fundraisingTargetDataForForm[x]);
    ...
    return {
        ...
        link: function(scope, element, attrs) {
            ...
            var loginUrl = attrs.loginUrl, fbStatus = null, fbLogin = null, loginToServer = function() {
                if (loginUrl) {
                    var auth = facebook.authenticationDetails().authResponse, formBody = angular.toJson({
                        accessToken: auth.accessToken,
                        userId: auth.userID,
                        signedRequest: auth.signedRequest
                    }), serverLogin = mHttp.post(loginUrl, {
                        data: formBody,
                        headers: {
        ...
        link: function(scope, element, attrs) {
            ...
            setProgressState("ready"), element.on("submit", function() {
                if (!serialized || !mHttp.isInProgress(formSubmitRequest)) {
                    ...
                    if (window.File && window.FileReader) $timeout(scope.$apply(function() {
                        ...
                        var target = "form_target_iframe_" + uniqueId(), $targetIframe = $("<iframe/>", {
                            ...
                            $timeout(scope.$apply(function() {
                                ...
                                console.log("RESPONSE", response), handleResponse(JSON.parse(response), originalData);
                            }));
                        }), $postForm = $("<form></form>", {
                            target: target,
                            method: "post",
                        ...
                        element.appendTo($postForm), $postForm.attr("action", attrs.action), $postForm.submit();
                    }
                    return !1;
                }
            });
    ...
    function kcRotate(elem, deg) {
        ...
        1 == dummy.getElementsByTagName("br").length && (elem.style.filter = "none"), dummy.parentNode.removeChild(dummy), 
        dummy = null;
    }
    var TOTAL_COUNT = 0;
    return {
    ...
    return {
        link: function(scope, element, attrs) {
            var hasInited = !1, isActive = !1, myParent = angular.element(element).parent(), reAlign = debounce(function() {
                element.css("top", (parentHeight() - myHeight()) / 2);
            }, 200), parentHeight = function() {
                return myParent.height() + Number(myParent.css("padding-top").replace("px", ""));
            }, myHeight = function() {
            ...
            attrs.$observe("mVerticalAlign", function(newVal) {
                hasInited || "true" !== myValue && myValue !== !0 || (isActive = !0, angular.element($window).bind("resize", reAlign), 
                reAlign(), $timeout(reAlign, 500)), hasInited || (hasInited = !0), newVal === !1 && (isActive = !1);
    ...
    return function(current, total, capAt100) {
        ...
        total = Number(total), isNaN(current) || isNaN(total) || (p = Math.floor(current / total * 100), 
    ...
    var FIND_BY_POSTCODE = "/umbraco/Api/PostcodeAnywhereApi/FindByPostcode", RETRIEVE_BY_ID = "/umbraco/Api/PostcodeAnywhereApi/RetrieveById";
    return {
        ...
        getAddressFromIdField: function(idField) {
            ...
            return formSubmitRequest.then(function(response) {
                response && response.length ? deferred.resolve(response[0]) : deferred.reject();
            }, function() {
                deferred.reject();
            }), promise;
        }
