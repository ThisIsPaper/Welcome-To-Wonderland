...
!function(a) {
    ...
    function jqLiteRemoveClass(element, cssClasses) {
        msie && 8 > msie ? cssClasses && forEach(cssClasses.split(" "), function(cssClass) {
    ...
    function jqLiteAddClass(element, cssClasses) {
        if (msie && 8 > msie) cssClasses && forEach(cssClasses.split(" "), function(cssClass) {
    ...
    ocModal.factory("$ocModal", [ "$rootScope", "$controller", "$location", "$timeout", "$compile", "$sniffer", "$q", function($rootScope, $controller, $location, $timeout, $compile, $sniffer, $q) {
        ...
        var self = {
            ...
            close: function(id) {
                ...
                return modal && modal.$scope.modalShow === !0 ? $timeout(function() {
...
wonderlandApp.config([ "$provide", function($provide) {
    ...
    }), $rootScope.currentPageUrl = $window.location.href, $rootScope.openPageModal = function(partial, modalId, initVars, controller) {
        if (partial) {
            var modal = {
    ...
    $scope.fixedTargets = [ 180, 250, 400 ], $scope.showChoose = $scope.fixedTargets.indexOf($init.fundraisingTargetData.FundraisingTarget) < 0;
    ...
    runScrollCheck(), angular.element($window).bind("scroll", runScrollCheck), $scope.onPageLoadProfileImage = function(profileImageData) {
        ...
        $timeout(function() {
            $scope.pageFeedback.profileImageProcessing = !1, response && response.Success === !0 ? ($scope.pageProfile.image = angular.fromJson(response.Message), 
    ...
    $scope.fundraisingTargetDataInit = function(fundraisingTargetData) {
        ...
        fundraisingTargetFirstSet && (fundraisingTargetFirstSet = !1, fundraisingTargetData.FundraisingTarget <= 0 && $timeout(function() {
            $rootScope.openPageModal("fundraisingTargetPartial", "fundraisingTargetModal", {
                fundraisingTargetData: $scope.fundraisingTargetDataForForm,
                hardcodedFundraisingTargets: $rootScope.hardcoded.fundraisingTargets,
                feedback: $scope.fundraisingTargetDataFeedback
            }, "FundraisingTargetCtrl");
        }));
    ...
    initialMoment.isValid() && ($scope.tempModel.partyDate = initialMoment, $scope.tempModel.partyTime = $scope.tempModel.times[$scope.tempModel.times.indexOf(initialMoment.format("HH:mm"))]), 
    ...
    $scope.wall = {
        ...
        $timeout(function() {
            $scope.wall.formModel = angular.copy(formModel), $scope.wall.formModel.PartyWallImage = null, 
            $scope.wall.previewImageUrl = null, $scope.wall.feedback.imageProcessing = !1, $scope.wall.feedback.messageProcessing = !1;
        });
...
} ]), wonderlandApp.directive("mDateTimePicker", [ "$timeout", function() {
    return {
        scope: {
    ...
    return {
        ...
        link: function(scope, element, attrs) {
            var onStartEvent = attrs.mOnStartEvent, onSuccessEvent = attrs.mOnSuccessEvent, resetInputValue = function() {
                console.log("resetInputValue::form input", element.find("input[type=file]")), element.find("input[type=file]").val(null);
            };
    ...
    return {
        ...
        link: function(scope, element) {
            scope.$watch("mLinkTarget", function(link) {
                $(element).attr("href", link.link), $(element).attr("target", link.target);
            });
        }
    };
    ...
    return {
        ...
        link: function(scope, element, attrs) {
            function handleResponse(response, originalData) {
                response && response.data && dataVar.assign(scope.$parent, response.data), response && response.Success === !0 ? (setProgressState("saved"), 
                savedConfirmTimer = $timeout(function() {
                    setProgressState("ready"), savedConfirmTimer = null;
                }, SAVED_CONFIRMATION_PERIOD), onSuccessEvent && $rootScope.$emit(onSuccessEvent, response, angular.copy(originalData))) : (emitError(), 
            ...
            setProgressState("ready"), element.on("submit", function() {
                if (!serialized || !mHttp.isInProgress(formSubmitRequest)) {
                    ...
                    if (!useLegacy && window.File && window.FileReader) $timeout(function() {
                        ...
                        var target = "form_target_iframe_" + uniqueId(), $targetIframe = $('<iframe name="' + target + '"></iframe>', {
                            id: target,
                            frameborder: "0"
                            ...
                            $timeout(function() {
                                var response = $targetIframe.contents().find("body").html().replace(/<\/?[^>]+>/gi, ""), parsedJson = null;
                                try {
                                    parsedJson = JSON.parse(response);
                                } catch (er) {}
                                console.log("response", response, parsedJson), handleResponse(parsedJson, originalData);
                            });
                        }), $postForm = $("<form></form>", {
                            target: target,
                            method: "post",
    ...
    return {
        ...
        link: function(scope, element, attrs) {
            ...
            attrs.$observe("mRaised", function(raised) {
                scope.raised = scope.$parent.$eval(raised), updatePercentage();
    ...
    return function(str, limit, suffix) {
        var newStr = "";
        return str && (newStr = $filter("limitTo")(str, limit) + (str.length > limit && suffix ? suffix : "")), 
        newStr;
    };
    ...
    return {
        backgroundUpload: function(form) {
            var deferred = $q.defer(), formTarget = "frame_form_target_" + uniqueId(), targetIframe = $('<iframe name="' + formTarget + '"></iframe>', {
                id: formTarget
